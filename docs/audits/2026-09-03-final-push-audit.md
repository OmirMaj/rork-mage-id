...
- **DB-F11b · P2 · SIBLING, NOT FIXED** — The same defect class on a parallel bucket, filed separately so DB-F11's fix cannot be read as closing it. `rfp-attachments` is also PUBLIC (5 live objects) and `utils/storage.ts:204` `uploadRfpAttachment` returns `getPublicUrl(path)`; `app/post-rfp.tsx` `pickDrawings` accepts `application/pdf` and `image/*` as `kind: 'drawing'` and writes those permanent URLs into `public_bids.drawing_urls`. `public_bids_select … TO authenticated USING (true)` (`supabase/schema.sql:3438`) lets ANY signed-in account — a free account anyone can register — enumerate every row and read every `drawing_url`, and because the bucket is public those URLs then resolve for a completely unauthenticated third party, forever, with no revocation. So after DB-F11 is applied, construction drawings are private *from the `plan-sheets` bucket*; they are not private from this one. Fix: private bucket + signed URLs at read time + a policy on `public_bids.drawing_urls`. Note the marketplace intent here differs from plans — an RFP listing is meant to be browsable by contractors — so this is a product decision as much as a security one, which is why it is not folded into DB-F11.
