...
- [ ] **No specific competitor dollar figures:** use a FIXED-STRING scan (the ERE form `grep -rnE "\$499…"` is buggy — inside double quotes `\$` collapses to a literal `$` that ERE treats as an end-of-line anchor, so it falsely returns empty). Run: `grep -rn -F -e '$499' -e '$1,099' -e '$50k' -e '$80M' -e '$80 million' -e '$1,000-a-month' -e '$50,000' -e '$4,500' -e '$10,000' -e '$80,000' *.html compare/*.html features/*.html` → **no output.** (MAGE's own `$29`/`$79` are fine; the `$50M+ revenue` customer-sizing descriptor in `compare/procore.html` is acceptable — it's not a price.)
