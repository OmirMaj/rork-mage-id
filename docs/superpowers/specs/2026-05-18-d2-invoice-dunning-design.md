...
- **Recipient:** resolve the project's client/homeowner email from `projects.client_portal.invites[].email` exactly as `homeowner-weekly-digest` does (same lookup + same "no email → skip" guard). One email per eligible invoice to that client.
- **Email:** compose via `wrapEmailHtml({ ...content, unsubscribe: { category: 'payment_reminders', ... } })` + `resendSend(RESEND_API_KEY, { to, ... })`, mirroring `homeowner-weekly-digest`'s `sendEmail`/`resendSend` call (so List-Unsubscribe/footer/`email_unsubscribes` compliance is inherited identically — D2 does NOT re-implement unsubscribe). Content: project name, invoice #, amount outstanding (`total_due - coalesce(amount_paid,0)`), days overdue, due date; stage-appropriate subject/tone; a primary `emailButton('View invoice', <portal invoice URL>)` linking to the existing homeowner portal (where the portal already surfaces a one-tap Pay Now when a Stripe link exists). **NOTE: there is no `pay_link_url` column on `invoices`** (verified live — the Stripe link is not a queryable invoice column), so the email links to the portal invoice view generically rather than embedding a pay link. Service role reads invoices + project, writes ONLY the 2 marker columns; never mutates financial fields.
