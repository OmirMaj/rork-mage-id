...
3. Passcode is **defense-in-depth, enforced only when the caller supplies one**: if `p_passcode IS NOT NULL` and `client_portal->>'passcode'` is non-empty and they differ → deny. If `p_passcode` is null, do NOT deny on the passcode — the security boundary that actually closes the vuln is step 2 (correct enabled portalId) + step 4 (contract belongs to that project) + step 5 (server-constructed columns). **This is required, not optional:** the portal passcode is validated out-of-band by the `validate-portal-passcode` edge function and is *deliberately never bundled into the portal snapshot* (`utils/portalSnapshot.ts` omits it by design so it never travels in the URL/snapshot), so the portal client legitimately cannot supply `p_passcode` in the normal flow. A hard "reject when null" gate would permanently break contract signing for every passcode-protected portal (a flow that works today — the dropped RLS had no passcode check at all). This leniency mirrors `validate-portal-passcode`'s own model ("if not passcode-protected / cannot verify here, pass") and the passcode-protection UI gate still independently guards portal access. Plain equality when supplied (PostgREST/gateway per-IP throttling is the brute-force control; true constant-time is not meaningfully achievable through PostgREST and is disproportionate).
...
- The passcode value: confirmed — the validated passcode is **not** retained client-side (`utils/portalSnapshot.ts` deliberately omits `passcode` from the snapshot; the `validate-portal-passcode` gate validates then discards it). So the body uses `p_passcode: (data && data.passcode) || null`, which is null in the normal flow. Per §4.1 step 3 (revised), the RPC does not deny on a null passcode — the security boundary is portalId-scoping + contract-belongs-to-project + server-constructed columns; the passcode equality is enforced only if a caller ever does supply one. Do **not** add new passcode plumbing (YAGNI; spec §4.4 / §4.1-3 — the security gain over the current "no check at all" state is already large and the passcode UI gate still independently guards access).
