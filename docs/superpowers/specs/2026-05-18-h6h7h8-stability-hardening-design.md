...
### 2026-05-18 — H6a list-virtualization deferred (the Gantt half was already done)

Resolved during implementation:
- **Gantt conditional mount:** verified ALREADY present in `app/(tabs)/schedule/index.tsx` at the pre-batch HEAD (`{viewMode === 'gantt' && …}` gates `<GanttChart>`/`<VerticalGantt>` in both desktop and mobile branches). No change required — H6a's heavy-component-eager-mount concern is already satisfied in the codebase.
- **List virtualization: DEFERRED.** The screen is a single outer `ScrollView` that owns the scroll for all 6 view modes + all chrome; the task lists render via `.map()` within it. True virtualization requires the list to be the scroller — i.e. promoting the screen to a `SectionList`/per-view scroller architecture across all 6 modes, a large structural refactor with real layout-regression risk across the whole screen. A `<FlatList scrollEnabled={false}>` nested in the outer `ScrollView` does NOT virtualize (RN renders all rows + emits the nested-VirtualizedList warning) — it was attempted (`4514804`) and **reverted** (`2cfe2a9`) as non-functional churn. Per the spec's proportionality + risk-fallback, list virtualization is deferred to a dedicated schedule-screen scroll-architecture effort (its own spec/plan), out of scope for this stability batch. The schedule lists are not crashing today; this is a perf-debt deferral, honestly recorded, not a regression.
