...
- Surface **start date earlier**: add the start-date control to the Tasks step (or a lightweight Step 0 "Project & start date"), so users aren't forced to Step 3 to discover the CPM anchor (audit finding #7). Keep the existing working-days explainer.
