...
const KINDS = new Set(['cost','contact','sub_compliance','permit','warranty','file_only']);
...
assert(resolveDestination('permit').folder === 'permits', 'permit files to Permits');
assert(resolveDestination('warranty').recordKind === 'warranty', 'warranty→warranty record');
assert(resolveDestination('warranty').folder === 'closeout', 'warranty files to Closeout');
