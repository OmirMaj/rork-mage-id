...
| `20260923101000_public_bids_private_columns.sql` *(wave 5, rfp-marketplace, #13/#86)* | Revokes table-level SELECT on `public_bids` from `anon` / `authenticated` and re-grants every column except `address_line`, `latitude`, `longitude`, `contact_email`, `posted_by`, so no other account can read a homeowner's street address, exact pin or email. Every installed build from before the wave-5 OTA still reads those columns (`BidsContext` `select('*')`, Review bids' `address_line` with `.single()`, the Nearby RFP feeds' `latitude` / `longitude`), and Postgres refuses the whole query — so applied early it takes the RFP marketplace down on every un-updated phone (no Review bids, no award, no Nearby feed). | After `20260923100000_rfp_marketplace_hardening.sql` is applied (it adds `get_rfp_private` / `get_bid_contacts` and `lat_coarse` / `lng_coarse`) AND the wave-5 OTA has reached devices (every `public_bids` select at HEAD names only granted columns). The file's header has the reverse path. |
