...
| `20260913120000_portal_proposal_acceptance.sql` | Adds the portal's proposal-acceptance table + RPC, and closes the missing change-order ownership check on the two live `portal_submit_co_approval*` functions. The acceptance RPC reads the published proposal out of `portal_snapshots`, so it denies every acceptance until a snapshot **v12** row exists — which needs the OTA on the contractor's device AND a re-push. Section 3 tightens functions that are live. | After `marketing/portal/index.html` carrying the acceptance flow is deployed AND the v12 OTA has reached devices. Run the `change_order_approvals` orphan query in the file's Section 3 header against production first; expect zero rows. |
