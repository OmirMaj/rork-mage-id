...
export interface Project {
  ...
  /**
   * Structured address for the automation engine (permit/inspection
   * scheduling, zoning grounding). ADDITIVE — the free-text `location`
   * above stays the fallback; this is populated when the address is parsed
   * into components. `zoningDistrict` is TRUST-BUT-VERIFY: when derived it
   * carries `zoningSource: 'guess'` and MUST NOT drive downstream code
  ...
  structuredAddress?: {
    ...
    /** ISO timestamp the contractor confirmed the zoning district. Absent =
     *  unconfirmed; downstream automation stays blocked. */
    zoningConfirmedAt?: string;
...
export interface ScheduleTask {
  ...
  /**
   * True when this task was produced by the automation engine
   * (eventToScheduleWork factory) rather than authored by a human. Auto-tasks
   * land as DRAFT and are gated by the autonomy ladder — never silently
   * committed. Surfaced in the review sheet so the GC confirms before commit.
   */
  isAutoGenerated?: boolean;
  /**
   * Audit + idempotency handle for an auto-generated task. `feature` is the
   * originating engine (e.g. 'permitRoadmap'), `id` the source record's id
   * (e.g. a RoadmapInspection id). The factory is IDEMPOTENT on this ref:
   * re-running the same event never double-creates a task with the same
   * sourceEventRef.
   */
