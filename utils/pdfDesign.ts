...
// Optional disclaimer renders as a subtle italic line above the brand
...
// ─── Disclaimers ──────────────────────────────────────────────────────
// Centralized so we keep the legal copy consistent across docs. None of
// this is legal advice — it's a "we tried, please verify" liability
// reducer recommended by every contractor who's ever been to court.
// Update copy here, not in individual builders.
export const PDF_DISCLAIMERS = {
  // Generic: appears on most documents. Soft, conversational, but firm.
  general:
    'This document was generated by MAGE ID. Please review all amounts and details for accuracy before relying on it. The contractor named above is responsible for the accuracy of all figures.',

  // Invoice — adds a line about payment processor + dispute window.
  invoice:
    'This invoice was generated by MAGE ID. Please verify all line items and totals before payment. Online payments are processed by Stripe; deposits are made directly to the contractor. Contact the contractor with any disputes within 10 days of payment.',

  // Daily field report — frames it as a record, not a certification.
  dfr:
    'This daily field report records on-site observations as documented by the contractor. It is not a certification of code compliance, structural sufficiency, or scope completion.',

  // Change order — emphasizes that signatures are binding.
  changeOrder:
    'A change order, once signed, is a binding amendment to the original contract. Please review every line item, amount, and schedule impact carefully. Consult your owner / architect / attorney if anything is unclear before signing.',

  // AIA G702/G703 — the heaviest disclaimer; the form is regulated.
  aia:
    'This is an AIA-style draft pay application generated by MAGE ID. AIA® and "AIA Document G702/G703" are registered trademarks of The American Institute of Architects, which is not affiliated with MAGE ID. Some lenders and architects require the official AIA Contract Documents. Verify all amounts, retainage, and certification language before submission. The contractor is solely responsible for the accuracy of every figure on this document.',

  // Closeout packet — handoff disclaimer (warranty + manuals reference).
  closeout:
