...
import { X, MapPin, ClipboardList, Camera, ImagePlus, RotateCcw, FileImage, CloudUpload } from 'lucide-react-native';
...
export default function PlanPinStep(props: PlanPinStepProps) {
  ...
  return (
    <Modal visible={visible} animationType="slide" presentationStyle="fullScreen" onRequestClose={onClose}>
      {visible ? <PlanPinStepBody {...props} /> : null}
    </Modal>
  );
}

// The body mounts fresh on every open, so its state (sheet, pin, errors) starts
// from the props each time instead of carrying a pin from the previous photo.
function PlanPinStepBody({
  ...
  const sheets = useMemo(() => {
    ...
    return offer;
  }, [allSheets, projectId, initialPin?.sheetId, justAdded]);

  const [sheetId, setSheetId] = useState<string | null>(() => {
    if (initialPin && allSheets.some(s => s.id === initialPin.sheetId)) return initialPin.sheetId;
    return choosePinSheet({ sheets: allSheets, projectId, sessionSheetId, punchItems });
  });
  ...
  const existing = useMemo(
    () => pinsOnSheet(punchItems, projectId, sheetId, sessionItemIds),
    [punchItems, projectId, sheetId, sessionItemIds],
  );
  ...
  // ── Render ───────────────────────────────────────────────────────────────
  const renderAddButtons = (verb: string) => (
  ...
  if (!sheet) {
    body = (
      <View style={styles.panel} testID="walk-pin-no-plan">
    ...
    body = (
      <View style={styles.panel} testID="walk-pin-sheet-missing">
          ...
          >
            <View
              ...
              accessibilityHint={pin
                ? 'Pin placed. Use Next to continue, or Skip to save without a pin.'
                : 'Double-tap to drop a pin in the middle of the plan, or Skip to save without a pin.'}
                ...
                return (
                  <View
                    ...
                    style={[
                      ...
                      { left: pos.left, top: pos.top, opacity: p.fromThisWalk ? 0.7 : 0.4 },
                    ]}
                  >
    ...
    <View style={[styles.root, { paddingTop: insets.top }]} testID="walk-pin-step">
      <View style={styles.header}>
        <TouchableOpacity
          ...
          accessibilityLabel="Back to the walk, keep the photo"
          testID="walk-pin-close"
        >
          <X size={20} color={t.text} strokeWidth={1.75} />
        </TouchableOpacity>
        <View style={{ flex: 1 }}>
          <Text style={styles.title}>Where is this?</Text>
          {label && <Text style={styles.subtitle} numberOfLines={1}>{label}</Text>}
        </View>
      ...
      <View style={[styles.footer, { paddingBottom: insets.bottom + 12 }]}>
        ...
        <View style={styles.footerRow}>
          <View style={{ flex: 1 }}>
            <Button label="Skip" variant="secondary" onPress={handleSkip} fullWidth testID="walk-pin-skip" />
          </View>
            ...
            <View style={{ flex: 2 }}>
              <Button
                ...
                label={pin && canPin
                  ? 'Next'
                  : mode === 'save'
                    ? 'Save the plan to pin on it'
                    : mode === 'repick'
                      ? 'Add the plan image to pin on it'
                      : loadState === 'error'
                        ? 'Plan didn’t load — Skip or try again'
                        : loadState === 'loading'
                          ? 'Loading the plan…'
...
const makeStyles = (t: ThemeColors) => StyleSheet.create({
  ...
  panel: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 10, paddingHorizontal: 28 },
