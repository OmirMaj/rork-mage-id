...
import {
  ...
  type EstimatePriceSource, type LinkedEstimateItemW5,
...
export default function EstimateCopilotReview({ draft, ctx, onBuild, onDiscard, patchDraft, note }: {
  ...
  const preview = useMemo(() => {
    ...
    const groups = new Map<string, LinkedEstimateItemW5[]>();
    for (const it of est.items as LinkedEstimateItemW5[]) {
      const k = it.category || 'General';
      groups.set(k, [...(groups.get(k) ?? []), it]);
    }
    const sources = (priced.costItems as LinkedEstimateItemW5[]).map((i) => i.priceSource ?? 'regional');
      ...
      <View style={styles.wrap}>
        ...
        {price.kind === 'error' ? (
          <>
            ...
            {price.limit ? (
              <TouchableOpacity style={styles.primary} activeOpacity={0.9} onPress={() => { onDiscard(); router.push('/paywall' as never); }} testID="copilot-estimate-see-plans">
                <Text style={styles.primaryText}>See plans</Text>
              </TouchableOpacity>
            ) : (
              <TouchableOpacity style={styles.primary} activeOpacity={0.9} onPress={() => void runPrice()} testID="copilot-estimate-retry">
        ...
        <View style={styles.totals}>
          ...
          <View style={styles.chips}>
            {MARKUP_CHOICES.map((n) => (
              <TouchableOpacity key={n} style={styles.chip} onPress={() => patchDraft({ markupPct: n })} activeOpacity={0.8} testID={`copilot-estimate-markup-${n}`}>
...
function makeStyles(colors: ThemeColors) {
  return StyleSheet.create({
    ...
    chip: { paddingHorizontal: Tokens.spacing.md, paddingVertical: Tokens.spacing.xs, borderRadius: Tokens.radius.full, borderWidth: 1, borderColor: colors.accent, backgroundColor: colors.surface },
    ...
    warn: { ...Type.footnote, color: colors.text, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.accent, borderRadius: Tokens.radius.md, padding: Tokens.spacing.sm },
