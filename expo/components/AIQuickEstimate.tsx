...
export default React.memo(function AIQuickEstimate({
  ...
  const handleApply = useCallback(() => {
    ...
    const safeMaterials = Array.isArray(result.materials) ? result.materials : [];
    const safeLabor = Array.isArray(result.labor) ? result.labor : [];
    const safeAssemblies = Array.isArray(result.assemblies) ? result.assemblies : [];
    const materialItems: CartItem[] = safeMaterials.map(aiMat => {
    ...
    const laborItems: LaborCartItem[] = safeLabor.map(aiLab => {
    ...
    const assemblyItems: AssemblyCartItem[] = safeAssemblies
      .map(aiAsm => {
  ...
  const estimatedTotals = useMemo(() => {
    ...
    const add = result.additionalCosts ?? { permits: 0, dumpsterRental: 0, equipmentRental: 0, cleanup: 0, contingencyPercent: 10, overheadPercent: 10 };
    ...
    const contingency = subtotal * ((add.contingencyPercent || 10) / 100);
    const overhead = subtotal * ((add.overheadPercent || 10) / 100);
    ...
    return (
      <Animated.View style={[s.resultContainer, { opacity: fadeAnim }]}>
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          {renderCollapsible('materials', `Materials (${(Array.isArray(result.materials) ? result.materials : []).length})`, Package, Colors.primary, () => (
            <View style={s.itemsList}>
              {(Array.isArray(result.materials) ? result.materials : []).map((m, i) => (
          ...
          {renderCollapsible('labor', `Labor (${(Array.isArray(result.labor) ? result.labor : []).length})`, HardHat, Colors.accent, () => (
            <View style={s.itemsList}>
              {(Array.isArray(result.labor) ? result.labor : []).map((l, i) => (
                <View key={i} style={s.itemRow}>
                  <View style={s.itemLeft}>
          ...
          {(Array.isArray(result.assemblies) ? result.assemblies : []).length > 0 && renderCollapsible('assemblies', `Assemblies (${(Array.isArray(result.assemblies) ? result.assemblies : []).length})`, Boxes, Colors.info, () => (
            <View style={s.itemsList}>
              {(Array.isArray(result.assemblies) ? result.assemblies : []).map((a, i) => (
                <View key={i} style={s.itemRow}>
                  <View style={s.itemLeft}>
          ...
          {renderCollapsible('additional', 'Additional Costs', DollarSign, Colors.textSecondary, () => (
            <View style={s.itemsList}>
              ...
              <View style={s.itemRow}>
                <Text style={s.itemName}>Contingency ({result.additionalCosts?.contingencyPercent ?? 10}%)</Text>
                <Text style={s.itemPrice}>${((estimatedTotals.materials + estimatedTotals.labor + estimatedTotals.additional) * (result.additionalCosts?.contingencyPercent ?? 10) / 100).toLocaleString(undefined, { maximumFractionDigits: 0 })}</Text>
              </View>
              <View style={s.itemRow}>
                <Text style={s.itemName}>Overhead ({result.additionalCosts?.overheadPercent ?? 10}%)</Text>
                <Text style={s.itemPrice}>${((estimatedTotals.materials + estimatedTotals.labor + estimatedTotals.additional) * (result.additionalCosts?.overheadPercent ?? 10) / 100).toLocaleString(undefined, { maximumFractionDigits: 0 })}</Text>
              </View>
            </View>
          ))}
          ...
          {(Array.isArray(result.savingsTips) ? result.savingsTips : []).length > 0 && (
            <View style={s.tipsCard}>
              <View style={s.tipsHeader}>
                <TrendingDown size={14} color={Colors.success} />
                <Text style={s.tipsTitle}>Savings Tips</Text>
              </View>
              {(Array.isArray(result.savingsTips) ? result.savingsTips : []).map((t, i) => (
