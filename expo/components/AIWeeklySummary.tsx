...
export default function AIWeeklySummary({ projects, visible, onClose }: Props) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      const safeData: WeeklySummaryResult = {
        weekRange: data?.weekRange ?? `Week of ${new Date().toLocaleDateString()}`,
        portfolioSummary: {
          totalProjects: data?.portfolioSummary?.totalProjects ?? projects.length,
          onTrack: data?.portfolioSummary?.onTrack ?? 0,
          atRisk: data?.portfolioSummary?.atRisk ?? 0,
          behind: data?.portfolioSummary?.behind ?? 0,
          combinedValue: data?.portfolioSummary?.combinedValue ?? 0,
          tasksCompletedThisWeek: data?.portfolioSummary?.tasksCompletedThisWeek ?? 0,
        },
