        ...
        <View style={[styles.modalContainer, { paddingTop: Platform.OS === 'ios' ? 12 : insets.top + 8 }]}>
          ...
          {result && (
            <ScrollView style={styles.scroll} contentContainerStyle={[styles.scrollContent, { paddingBottom: insets.bottom + 30 }]}>
              ...
              {(Array.isArray(result.keyAccomplishments) ? result.keyAccomplishments : []).length > 0 && (
                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>Key Accomplishments</Text>
                  {(Array.isArray(result.keyAccomplishments) ? result.keyAccomplishments : []).map((item, idx) => (
              ...
              {(Array.isArray(result.issuesAndRisks) ? result.issuesAndRisks : []).length > 0 && (
                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>Issues & Risks</Text>
                  {(Array.isArray(result.issuesAndRisks) ? result.issuesAndRisks : []).map((item, idx) => (
                    <View key={idx} style={styles.listRow}>
              ...
              {(Array.isArray(result.nextMilestones) ? result.nextMilestones : []).length > 0 && (
                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>Next Milestones</Text>
                  {(Array.isArray(result.nextMilestones) ? result.nextMilestones : []).map((item, idx) => (
                    <View key={idx} style={styles.listRow}>
              ...
              {(Array.isArray(result.recommendations) ? result.recommendations : []).length > 0 && (
                <View style={styles.section}>
                  ...
                  {(Array.isArray(result.recommendations) ? result.recommendations : []).map((item, idx) => (
                    <View key={idx} style={styles.listRow}>
