...
try {
  ...
  Purchases = RNP.default || RNP.Purchases;
  LOG_LEVEL = RNP.LOG_LEVEL ?? { DEBUG: 0 };
  console.log('[RC] react-native-purchases loaded successfully');
} catch (e) {
  console.warn('[RC] react-native-purchases not available — using free tier defaults');
}
...
function configureRC() {
  ...
  if (!Purchases) {
    console.log('[RC] Purchases module not available, skipping configuration');
    return;
  }
...
export const [SubscriptionProvider, useSubscription] = createContextHook(() => {
  ...
  const proPackage = useMemo(() => {
    return offeringsQuery.data?.current?.availablePackages.find(
      (p: any) => p.identifier === 'pro_monthly' || p.identifier === '$rc_monthly'
    ) ?? null;
  }, [offeringsQuery.data]);

  const businessPackage = useMemo(() => {
    return offeringsQuery.data?.current?.availablePackages.find(
      (p: any) => p.identifier === 'business_monthly'
    ) ?? null;
  }, [offeringsQuery.data]);
