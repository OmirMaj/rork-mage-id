...
export function buildCollaboratorInviteHtml(opts: {
  companyName: string;
  recipientName: string;
  projectName: string;
  ...
  const {
    companyName, recipientName, projectName, projectLocation,
    role, inviteLink, senderName, senderEmail, senderPhone,
  } = opts;

  const roleLabel = role === 'editor' ? 'an Editor (can edit)' : 'a Viewer (read-only)';
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
        <tr><td style="background:#1a6b3c;padding:28px 32px;">
          ...
          <p style="margin:4px 0 0;color:rgba(255,255,255,0.8);font-size:13px;">Project Collaboration Invite</p>
        </td></tr>
        <tr><td style="padding:32px;">
          <p style="margin:0 0 16px;color:#374151;font-size:16px;">Hi ${recipientName},</p>
          <p style="margin:0 0 20px;color:#374151;line-height:1.6;font-size:15px;">
            <strong>${senderName}</strong> has invited you to collaborate on a project in <strong>${companyName}</strong>.
          </p>
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#f0fdf4;border-radius:10px;margin:20px 0;border:1px solid #bbf7d0;">
            <tr><td style="padding:24px;">
              <p style="margin:0 0 4px;color:#6b7280;font-size:12px;text-transform:uppercase;letter-spacing:0.5px;">Project</p>
              <p style="margin:0 0 12px;color:#111827;font-size:18px;font-weight:700;">${projectName}</p>
              <table width="100%">
                <tr>
                  <td style="color:#6b7280;font-size:13px;padding:3px 0;">Location</td>
                  <td align="right" style="color:#111827;font-size:14px;padding:3px 0;">${projectLocation}</td>
                </tr>
                <tr>
                  <td style="color:#6b7280;font-size:13px;padding:3px 0;">Your Role</td>
                  <td align="right" style="color:#1a6b3c;font-size:14px;font-weight:600;padding:3px 0;">${roleLabel}</td>
                </tr>
              </table>
            </td></tr>
          </table>
          <table width="100%" cellpadding="0" cellspacing="0" style="margin:24px 0;">
            <tr><td align="center">
              <a href="${inviteLink}" style="display:inline-block;background:#1a6b3c;color:#ffffff;font-size:16px;font-weight:700;text-decoration:none;padding:14px 40px;border-radius:10px;">Open Project in MAGE ID</a>
            </td></tr>
          </table>
          <p style="margin:0 0 12px;color:#6b7280;font-size:13px;line-height:1.5;">
            If the button doesn't work, copy and paste this link:<br/>
            <a href="${inviteLink}" style="color:#1a6b3c;word-break:break-all;">${inviteLink}</a>
          </p>
          <hr style="border:none;border-top:1px solid #e5e7eb;margin:20px 0;"/>
          <p style="margin:0;color:#9ca3af;font-size:12px;line-height:1.5;">
            Sent via MAGE ID &mdash; Construction Project Management
            ${senderName ? `<br/>From: ${senderName}` : ''}
