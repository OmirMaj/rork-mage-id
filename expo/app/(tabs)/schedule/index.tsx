...
export default function ScheduleScreen() {
  ...
  const { projects, updateProject, addProject, settings } = useProjects();
...
Include a Project Start milestone (duration 0) and Project Complete milestone (duration 0). Group tasks into logical phases with realistic durations and dependencies. Generate 15-40 tasks depending on project size.`,
      ...
      console.log('[Schedule] AI data type:', typeof response, Array.isArray(response));
      if (typeof response === 'string') {
        console.log('[Schedule] AI data string preview:', response.substring(0, 300));
        ...
        try {
          ...
          if (objMatch) {
            try { response = JSON.parse(objMatch[0]); } catch { /* ignore */ }
          }
          if (typeof response === 'string' && arrMatch) {
            try { response = JSON.parse(arrMatch[0]); } catch { /* ignore */ }
          }
          if (typeof response === 'string') {
            console.log('[Schedule] Could not parse AI response:', cleaned.substring(0, 200));
      ...
      console.log('[Schedule] AI returned', result?.length, 'tasks');
