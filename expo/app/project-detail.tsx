...
export default function ProjectDetailScreen() {
  ...
  const handleInvite = useCallback(async () => {
    ...
    const htmlBody = buildCollaboratorInviteHtml({
      companyName: branding.companyName || 'MAGE ID',
      recipientName: name,
      projectName: project.name,
      projectLocation: project.location,
      role: inviteRole as 'editor' | 'viewer',
      inviteLink,
      senderName: branding.contactName || 'A team member',
      senderEmail: branding.email,
      senderPhone: branding.phone,
    });

    try {
      const apiBaseUrl = process.env.EXPO_PUBLIC_RORK_API_BASE_URL;
      if (apiBaseUrl) {
        const resp = await fetch(`${apiBaseUrl}/email/send`, {
          method: 'POST',
        ...
        if (result.success) {
          console.log('[ProjectDetail] Invite email sent via API');
        } else {
          console.warn('[ProjectDetail] API email failed:', result.error);
          const { success: nativeOk } = await sendEmail({
            to: email,
            subject,
            html: htmlBody,
          });
          if (!nativeOk) {
            console.log('[ProjectDetail] Falling back to mailto');
            const mailUrl = `mailto:${email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(`You've been invited to collaborate on "${project.name}". Open the MAGE ID app to get started.`)}`;
      ...
      console.warn('[ProjectDetail] Email send error:', err);
      const mailUrl = `mailto:${email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(`You've been invited to collaborate on "${project.name}". Open the MAGE ID app to get started.`)}`;
      Linking.openURL(mailUrl).catch(() => {});
    } finally {
    ...
    if (Platform.OS !== 'web') void Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    Alert.alert('Invited', `Invitation sent to ${name} (${email}) as ${inviteRole}.`);
  ...
  const pendingCOs = useMemo(() => changeOrders.filter(co => co.status === 'submitted' || co.status === 'under_review'), [changeOrders]);
  const totalApprovedCOAmount = useMemo(() => approvedCOs.reduce((sum, co) => sum + co.changeAmount, 0), [approvedCOs]);
  const totalPendingCOAmount = useMemo(() => pendingCOs.reduce((sum, co) => sum + co.changeAmount, 0), [pendingCOs]);
  const adjustedContractValue = useMemo(() => heroTotal + totalApprovedCOAmount, [heroTotal, totalApprovedCOAmount]);
  const totalInvoiced = useMemo(() => projectInvoices.reduce((sum, inv) => sum + inv.totalDue, 0), [projectInvoices]);
  const totalPaid = useMemo(() => projectInvoices.reduce((sum, inv) => sum + inv.amountPaid, 0), [projectInvoices]);
  const totalOutstanding = useMemo(() => totalInvoiced - totalPaid, [totalInvoiced, totalPaid]);
  const remainingToBill = useMemo(() => Math.max(0, adjustedContractValue - totalInvoiced), [adjustedContractValue, totalInvoiced]);
      ...
      >
        ...
        <View style={styles.section}>
          <TouchableOpacity
            ...
            activeOpacity={0.7}
            testID="finance-overview-section"
          >
            <Wallet size={20} color={Colors.success} />
            <Text style={styles.sectionTitle}>Financial Overview</Text>
            {totalApprovedCOAmount !== 0 && (
              <View style={styles.coAdjustBadge}>
                <Repeat size={10} color={Colors.accent} />
                <Text style={styles.coAdjustBadgeText}>
                  {totalApprovedCOAmount >= 0 ? '+' : ''}{formatMoney(totalApprovedCOAmount)} COs
                </Text>
              </View>
            )}
            {expanded.budget ? (
          ...
          {expanded.budget && (
            <View style={styles.coCard}>
              <View style={styles.financeHubGrid}>
                <View style={styles.financeHubCell}>
                  <Text style={styles.financeHubCellLabel}>Contract Value</Text>
                  <Text style={styles.financeHubCellValue}>{formatMoney(adjustedContractValue)}</Text>
                  {totalApprovedCOAmount !== 0 && (
                    <Text style={styles.financeHubCellSub}>Orig: {formatMoney(heroTotal)}</Text>
                  )}
                </View>
                <View style={styles.financeHubCell}>
                  <Text style={styles.financeHubCellLabel}>Invoiced</Text>
                  <Text style={[styles.financeHubCellValue, { color: Colors.info }]}>{formatMoney(totalInvoiced)}</Text>
                  <Text style={styles.financeHubCellSub}>{projectInvoices.length} invoice{projectInvoices.length !== 1 ? 's' : ''}</Text>
                </View>
                <View style={styles.financeHubCell}>
                  <Text style={styles.financeHubCellLabel}>Collected</Text>
                  <Text style={[styles.financeHubCellValue, { color: Colors.success }]}>{formatMoney(totalPaid)}</Text>
                  {totalOutstanding > 0 && (
                    <Text style={[styles.financeHubCellSub, { color: Colors.warning }]}>{formatMoney(totalOutstanding)} owed</Text>
                  )}
                </View>
                <View style={styles.financeHubCell}>
                  <Text style={styles.financeHubCellLabel}>Left to Bill</Text>
                  <Text style={[styles.financeHubCellValue, { color: remainingToBill > 0 ? Colors.accent : Colors.success }]}>{formatMoney(remainingToBill)}</Text>
                  {pendingCOs.length > 0 && (
                    <Text style={[styles.financeHubCellSub, { color: Colors.warning }]}>{pendingCOs.length} pending CO{pendingCOs.length !== 1 ? 's' : ''}</Text>
                  )}
                </View>
              </View>

              {adjustedContractValue > 0 && (
                <View style={styles.financeProgressWrap}>
                  <View style={styles.financeProgressHeader}>
                    <Text style={styles.financeProgressLabel}>Billed</Text>
                    <Text style={styles.financeProgressPct}>
                      {Math.min(100, Math.round((totalInvoiced / adjustedContractValue) * 100))}%
                    </Text>
                  </View>
                  <View style={styles.financeProgressTrack}>
                    <View style={[styles.financeProgressFillPaid, { width: `${Math.min(100, (totalPaid / adjustedContractValue) * 100)}%` as any }]} />
                    <View style={[styles.financeProgressFillOwed, { width: `${Math.min(100, (totalOutstanding / adjustedContractValue) * 100)}%` as any }]} />
                  </View>
                  <View style={styles.financeProgressLegend}>
                    <View style={styles.financeProgressLegendItem}>
                      <View style={[styles.financeProgressLegendDot, { backgroundColor: Colors.success }]} />
                      <Text style={styles.financeProgressLegendText}>Collected</Text>
                    </View>
                    <View style={styles.financeProgressLegendItem}>
                      <View style={[styles.financeProgressLegendDot, { backgroundColor: Colors.warning }]} />
                      <Text style={styles.financeProgressLegendText}>Outstanding</Text>
                    </View>
                    <View style={styles.financeProgressLegendItem}>
                      <View style={[styles.financeProgressLegendDot, { backgroundColor: Colors.fillTertiary }]} />
                      <Text style={styles.financeProgressLegendText}>Unbilled</Text>
                    </View>
                  </View>
                </View>
              )}
              ...
              {approvedCOs.length > 0 && (
                <View style={styles.coImpactCard}>
                  <Text style={styles.coImpactTitle}>Approved Change Orders Impact</Text>
                  <View style={styles.coImpactRow}>
                    <Text style={styles.coImpactLabel}>Original Estimate</Text>
                    <Text style={styles.coImpactValue}>{formatMoney(heroTotal)}</Text>
                  </View>
                  {approvedCOs.map(co => (
                    <View key={co.id} style={styles.coImpactRow}>
                      <Text style={styles.coImpactLabel}>CO #{co.number}</Text>
                      <Text style={[styles.coImpactValue, { color: co.changeAmount >= 0 ? Colors.accent : Colors.success }]}>
                        {co.changeAmount >= 0 ? '+' : ''}{formatMoney(co.changeAmount)}
                      </Text>
                    </View>
                  ))}
                  <View style={styles.coImpactDivider} />
                  <View style={styles.coImpactRow}>
                    <Text style={styles.coImpactTotalLabel}>Adjusted Total</Text>
                    <Text style={styles.coImpactTotalValue}>{formatMoney(adjustedContractValue)}</Text>
                  </View>
                </View>
              )}

              <View style={styles.financeHubActions}>
                <TouchableOpacity
                  style={styles.financeHubActionBtn}
                ...
                >
                  ...
                  <Text style={[styles.financeHubActionText, { color: Colors.success }]}>Cash Flow Forecast</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.financeHubActionBtn}
                  onPress={() => router.push({ pathname: '/budget-dashboard' as any, params: { projectId: id } })}
                  activeOpacity={0.7}
                  testID="project-budget-btn"
                >
                  <DollarSign size={18} color={Colors.primary} />
                  <Text style={[styles.financeHubActionText, { color: Colors.primary }]}>Budget Dashboard</Text>
                </TouchableOpacity>
              </View>
            </View>
          )}
        ...
        <View style={styles.section}>
          ...
          {expanded.schedule && project.schedule && (
            <View style={styles.scheduleCard}>
              <View style={{ flexDirection: 'row', gap: 8, marginBottom: 10 }}>
                <TouchableOpacity
                  style={[styles.coAddBtn, { flex: 1, marginTop: 0 }]}
                  onPress={() => router.push({ pathname: '/(tabs)/schedule' as any, params: { projectId: id } })}
                  activeOpacity={0.7}
                  testID="open-schedule-btn"
                >
                  <CalendarDays size={16} color={Colors.info} />
                  <Text style={[styles.coAddBtnText, { color: Colors.info }]}>Open Schedule</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[styles.coAddBtn, { flex: 1, marginTop: 0, backgroundColor: Colors.success + '10', borderColor: Colors.success + '20' }]}
                  onPress={async () => {
                    if (!project?.schedule) return;
                    try {
                      await generateScheduleSectionPdf(project.schedule, project.name, branding);
                    } catch (e) {
                      console.error('[ProjectDetail] Schedule export error:', e);
                      Alert.alert('Error', 'Failed to export schedule.');
                    }
                  }}
                  activeOpacity={0.7}
                  testID="export-schedule-btn"
                >
                  <Share2 size={16} color={Colors.success} />
                  <Text style={[styles.coAddBtnText, { color: Colors.success }]}>Export PDF</Text>
                </TouchableOpacity>
              </View>
          ...
          {expanded.schedule && !project.schedule && (
            <View style={styles.noScheduleCard}>
              <View style={styles.noScheduleIconWrap}>
                <CalendarDays size={28} color={Colors.info} />
              </View>
              <Text style={styles.noScheduleTitle}>No Schedule Yet</Text>
              <Text style={styles.noScheduleDesc}>
                Create a schedule to plan tasks, track progress, and manage your project timeline.
              </Text>
              <TouchableOpacity
                style={styles.createScheduleBtn}
                onPress={() => router.push({ pathname: '/create-schedule' as any, params: { projectId: id } })}
                activeOpacity={0.85}
                testID="create-schedule-btn"
              >
                <Plus size={16} color={Colors.textOnPrimary} />
                <Text style={styles.createScheduleBtnText}>Create Schedule</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.openScheduleTabBtn}
                onPress={() => router.push({ pathname: '/(tabs)/schedule' as any, params: { projectId: id } })}
                activeOpacity={0.7}
                testID="open-schedule-tab-btn"
              >
                <CalendarDays size={14} color={Colors.info} />
                <Text style={styles.openScheduleTabBtnText}>Open Schedule Planner</Text>
              </TouchableOpacity>
            </View>
          )}
        </View>
                ...
                <View key={collab.id} style={styles.collabMember}>
                  ...
                  <View style={styles.collabInfo}>
                    ...
                    {collab.invitedAt && (
                      <Text style={{ fontSize: 10, color: Colors.textMuted, marginTop: 1 }}>
                        Invited {new Date(collab.invitedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                      </Text>
                    )}
                  </View>
                  <View style={styles.collabActions}>
                    ...
                    {collab.status === 'pending' && (
                      <TouchableOpacity
                        style={[styles.collabRoleBadge, { backgroundColor: Colors.primary + '15' }]}
                        onPress={async () => {
                          if (!project) return;
                          const inviteLink = `https://mageid.app/invite/${id}?email=${encodeURIComponent(collab.email)}&role=${collab.role}`;
                          const subject = `${branding.companyName || 'MAGE ID'} - Reminder: Project Invitation: ${project.name}`;
                          const htmlBody = buildCollaboratorInviteHtml({
                            companyName: branding.companyName || 'MAGE ID',
                          ...
                          try {
                            ...
                            if (apiBaseUrl) {
                              await fetch(`${apiBaseUrl}/email/send`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ to: collab.email, subject, html: htmlBody }),
                              });
                            } else {
                              await sendEmail({ to: collab.email, subject, html: htmlBody });
                            }
                            if (Platform.OS !== 'web') void Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
                            Alert.alert('Resent', `Invitation resent to ${collab.name}.`);
                          } catch {
                        ...
                        activeOpacity={0.7}
                      >
                        <Text style={[styles.collabRoleText, { color: Colors.primary }]}>Resend</Text>
                      </TouchableOpacity>
                    )}
                    <TouchableOpacity
        ...
        {!hasAnyEstimate && (
          <View style={styles.noEstimate}>
            ...
            <Text style={styles.noEstimateText}>
              Create an estimate to track materials, labor, and costs for this project.
            </Text>
            <TouchableOpacity
              style={styles.createEstimateBtn}
              activeOpacity={0.7}
              onPress={() => router.push('/(tabs)/estimate')}
              testID="create-estimate-btn"
            >
              <Plus size={18} color={Colors.textOnPrimary} />
              <Text style={styles.createEstimateBtnText}>Create Estimate</Text>
            </TouchableOpacity>
          </View>
        )}
          ...
          >
            <View style={[styles.inviteModalCard, { paddingBottom: insets.bottom + 20 }]}>
              ...
              <View style={styles.inviteActionRow}>
                ...
                <TouchableOpacity style={[styles.inviteSendBtn, inviteSending && { opacity: 0.6 }]} onPress={handleInvite} activeOpacity={0.85} testID="send-invite-btn" disabled={inviteSending}>
                  {inviteSending ? (
                    <ActivityIndicator size="small" color={Colors.textOnPrimary} />
                  ) : (
                    <Send size={16} color={Colors.textOnPrimary} />
                  )}
                  <Text style={styles.inviteSendBtnText}>{inviteSending ? 'Sending...' : 'Send Invite'}</Text>
                </TouchableOpacity>
              </View>
            </View>
          </ScrollView>
...
const styles = StyleSheet.create({
  ...
  noScheduleCard: { backgroundColor: Colors.card, borderRadius: 12, marginTop: 8, borderWidth: 1, borderColor: Colors.cardBorder, padding: 24, alignItems: 'center' as const, gap: 8 },
  noScheduleIconWrap: { width: 56, height: 56, borderRadius: 18, backgroundColor: Colors.info + '12', alignItems: 'center' as const, justifyContent: 'center' as const, marginBottom: 4 },
  noScheduleTitle: { fontSize: 17, fontWeight: '700' as const, color: Colors.text },
  noScheduleDesc: { fontSize: 14, color: Colors.textSecondary, textAlign: 'center' as const, lineHeight: 20, paddingHorizontal: 10 },
  createScheduleBtn: { flexDirection: 'row' as const, alignItems: 'center' as const, justifyContent: 'center' as const, gap: 6, backgroundColor: Colors.info, borderRadius: 12, paddingVertical: 14, paddingHorizontal: 24, marginTop: 8, width: '100%' as any },
  createScheduleBtnText: { fontSize: 15, fontWeight: '700' as const, color: Colors.textOnPrimary },
  openScheduleTabBtn: { flexDirection: 'row' as const, alignItems: 'center' as const, justifyContent: 'center' as const, gap: 6, paddingVertical: 12, borderRadius: 10, backgroundColor: Colors.info + '10', borderWidth: 1, borderColor: Colors.info + '20', width: '100%' as any },
  openScheduleTabBtnText: { fontSize: 13, fontWeight: '600' as const, color: Colors.info },
  ...
  createEstimateBtn: { flexDirection: 'row' as const, alignItems: 'center' as const, justifyContent: 'center' as const, backgroundColor: Colors.primary, borderRadius: 12, paddingVertical: 14, paddingHorizontal: 24, marginTop: 16, gap: 8 },
  createEstimateBtnText: { fontSize: 16, fontWeight: '600' as const, color: Colors.textOnPrimary },
  ...
  financeProgressFillPaid: {
    height: 8,
    backgroundColor: Colors.success,
    borderRadius: 4,
  },
  financeProgressFillOwed: {
    height: 8,
    backgroundColor: Colors.warning,
  },
  financeProgressLegend: {
    flexDirection: 'row',
    gap: 12,
  },
  financeProgressLegendItem: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
  },
  financeProgressLegendDot: {
    width: 8,
    height: 8,
    borderRadius: 4,
  },
  financeProgressLegendText: {
    fontSize: 10,
    color: Colors.textMuted,
    fontWeight: '500' as const,
  },
