...
export type PortalRole = 'contractor' | 'client' | 'subcontractor';
...
export type ProfileAvailability = 'available' | 'busy' | 'not_taking_work';
...
export interface ContractorProfile {
  ...
  contactEmail: string;
  phone: string;
  contactVisibility: 'public' | 'connections_only' | 'hidden';
  rating: number;
  reviewCount: number;
  createdAt: string;
  updatedAt: string;
...
export interface CustomTemplate {
  id: string;
  name: string;
  type: 'proposal' | 'schedule' | 'invoice' | 'daily_report';
  mode: 'static' | 'dynamic';
  fileUri?: string;
  htmlContent?: string;
  createdAt: string;
