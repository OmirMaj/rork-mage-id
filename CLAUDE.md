...
- **Persistence**: `AsyncStorage`. Keys are namespaced under `mageid_*` — core (`mageid_projects`, `mageid_settings`, `mageid_user_role`), project sub-collections (`mageid_change_orders`, `mageid_invoices`, `mageid_daily_reports`, `mageid_punch_items`, `mageid_photos`, `mageid_rfis`, `mageid_submittals`, `mageid_warranties`, `mageid_portal_messages`), and app state (`mageid_offline_queue`, `mageid_theme`, `mageid_auth_*`) — plus a second live `mage_*` namespace (`mage_cashflow_data`, `mage_bids`, `mage_voice_history`, `mage_ai_*`) and three un-namespaced strays (`bids_*`, `post-rfp:draft:*`). Historically core keys used `buildwise_*` and sub-collections used `tertiary_*`; both were renamed to `mageid_*` in the pre-launch de-brand (`0dfbce0`), which left the old keys sitting on disk and out of the wipe path — a cross-tenant leak that the 2026-08-17 web audit caught in production.
- **Tenant boundary on local storage**: `utils/localCacheKeys.ts` is the single source of truth for which keys the app owns and which survive a tenant switch. `wipeLocalUserCache` (`contexts/AuthContext.tsx`) sweeps by **prefix** over `getAllKeys()`, so a new key is covered the moment it is written — **you no longer add keys to `LOCAL_USER_CACHE_KEYS`** (that hand-maintained list is now only a fallback for when `getAllKeys()` throws). What you *do* have to do: give any new key one of the prefixes in `APP_STORAGE_PREFIXES`. A key under a brand-new prefix is invisible to the sweep and leaks across tenants on a shared device — `bun run test:storage-hygiene` fails the build if you introduce one, if you reintroduce a `buildwise_*`/`tertiary_*` key, or if the sweep would touch a foreign key on the web origin. The sweep must never become `AsyncStorage.clear()`: on web `AsyncStorage` **is** `window.localStorage`, shared per origin, and `clear()` would take out Supabase's own `sb-<ref>-auth-token` session along with Stripe/RevenueCat/Sentry state.
